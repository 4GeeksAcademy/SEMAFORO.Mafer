    ...
    return (
        <div className="contenedor">
            <div className="semaforo">
                {colores.map(c => (<div key={c} className={`luz ${c} ${colorActivo === c ? 'activo' : ''}`} onClick={() => setColorActivo(c)}></div>))}
            </div>
            <div className="botonera"><br />
                <button onClick={alternarColor}>Cambiar</button>          
            </div>
        </div>
    );
}
